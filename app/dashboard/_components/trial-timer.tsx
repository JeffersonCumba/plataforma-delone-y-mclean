...
function getColorClasses(daysRemaining: number, isExpired: boolean, isWarningPeriod: boolean) {
...
function getTextColorClasses(isExpired: boolean, isWarningPeriod: boolean) {
  ...
  if (isWarningPeriod) return "text-amber-600";
...
const sizeConfig = {
  sm: { trackHeight: "h-2", font: "text-xs font-medium", gap: "gap-1.5", iconSize: "sm" as const },
  md: { trackHeight: "h-3", font: "text-sm font-medium", gap: "gap-2", iconSize: "md" as const },
  lg: { trackHeight: "h-4", font: "text-base font-semibold", gap: "gap-2.5", iconSize: "lg" as const },
};
...
export function TrialTimer({
  ...
  const progress = isExpired ? 0 : Math.max(0, Math.min(1, daysRemaining / trialDays));
  ...
  return (
    <div className={cn("flex items-center", cfg.gap, className)}>
      <div className="flex-1 min-w-0 relative" style={{ maxWidth: size === "sm" ? "120px" : size === "md" ? "160px" : "200px" }}>
        <div className={cn("relative rounded-full bg-slate-200 overflow-hidden", cfg.trackHeight)}>
          <div
            className={cn("absolute top-0 left-0 bottom-0 rounded-full transition-all duration-500", colorClass)}
            style={{ width: `${percentage}%` }}
          />
        </div>
      </div>
      <div className={cn("flex items-center gap-1.5 whitespace-nowrap", textColorClass)}>
      ...
      {trialEndsAt && !isExpired && showLabel && (
        <span className="text-[10px] text-slate-400 hidden md:inline-block ml-1 whitespace-nowrap">
          Expira: {trialEndsAt.toLocaleDateString("es-AR", { day: "numeric", month: "short", year: "numeric" })}
        </span>
      )}
    </div>
  );
